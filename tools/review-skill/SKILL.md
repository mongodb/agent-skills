...
description: >-
  ...
  quality before publishing. Runs the skill-validator-ent CLI to check for
...
compatibility: Requires skill-validator-ent CLI. LLM scoring requires AWS Bedrock access OR can be skipped for structural-only review.
metadata:
  author: mongodb
  version: "1.0"
---
...
> 3. **Change environment** — switch between Bedrock and structural-only
...
> LLM scoring uses Claude via AWS Bedrock. Without Bedrock, we run structural
> validation only.
>
> 1. **Yes, I have AWS Bedrock access** — full review with LLM scoring
> 2. **No, skip LLM scoring** — structural validation only

Option 1: set `LLM_SCORING=true`. After Step 1a, follow
[references/bedrock-llm-scoring.md](references/bedrock-llm-scoring.md) for
...
follow [references/install-skill-validator-ent.md](references/install-skill-validator-ent.md).

Do NOT proceed until this succeeds.

If `LLM_SCORING=true`, complete the Bedrock prerequisite checks in
[references/bedrock-llm-scoring.md](references/bedrock-llm-scoring.md) before
continuing.
...
sections of
[references/bedrock-llm-scoring.md](references/bedrock-llm-scoring.md).
