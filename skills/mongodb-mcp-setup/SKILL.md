...
Add the environment variables to the appropriate profile file. Add a comment to make it clear what these are for. Adapt the syntax to match the user's detected shell (e.g. `export` for bash/zsh, `set -gx` for fish, `$env:` assignments for PowerShell).
